...
utils.getBucketNameFromHost = function getBucketNameFromHost(request) {
    ...
    // All endpoints from all regions + `s3-website-$ENDPOINT...`
    // or if config.websiteEndpointPrefix is specified (e.g., 'scality-website')
    // it will be whatever was specified plus each endpoint
    const validHosts = utils.getAllEndpoints().concat(
        utils.getAllEndpoints().map(r => `${config
            .websiteEndpointPrefix}-${r}.amazonaws.com`));
