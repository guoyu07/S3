...
import { isBucketAuthorized, isObjAuthorized, isMpuAuthorized } from
...
function generateMPUList(response, splitter) {
    ...
    metadataValidateAuthorization(params, cb) {
        ...
        (err, data) => {
            ...
            return async.waterfall([
                next => validateBucket(params, data, canonicalID, next),
                (bucket, data, next) =>
                    validateObject(params, bucket, data, canonicalID, next),
            ], (err, bucket, obj) => {
                if (err) {
        ...
        metadata.getBucket(mpuBucketName, log, (err, mpuBucket) => {
            ...
            metadata.listObject(mpuBucketName, searchArgs,
                log, (err, response) => {
                    ...
                    const mpuOverview = generateMPUList(response, splitter);
