...
export function isMpuAuthorized(mpuBucket, authInfo, initiatorID, ownerID,
    requestType) {
    ...
    if (requestType === 'deleteMPU' || requestType === 'listParts') {
        // In order for account/user to be authorized must either be the
        // bucket owner or intitator of the multipart upload request
        // (or parent account of initiator). In addition if the bucket policy
        // designates someone else with s3:AbortMultipartUpload or
        // s3:ListMultipartUploadPartsrights, as applicable, that account/user
        // will have the right. If got to this step, it means there is
